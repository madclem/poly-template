...
void main(void) {
	...
	if (shininess < 255.0) {
		...
		float specularLightWeighting = pow(max(dot(reflectionDirection, eyeDirection), 0.0), 0.);
	...
	gl_FragColor = vec4(textureColor.rgb * lightWeighting, textureColor.a * specularLightWeighting);
